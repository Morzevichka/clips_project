...
class Net_MN_Large(nn.Module):
    ...
    The model uses:
    - MobileNetV3_Large as the feature extractor for video data.
    - GRU layers to process video and audio features over time.
    - CNN layers to process audio features.
...
class Net_EN_B0(nn.Module):
    ...
    def __init__(self):
        ...
        self.video_cnn = efficientnet

        self.video_rnn = nn.GRU(input_size=1280, hidden_size=512, num_layers=2, batch_first=True, bidirectional=True)
