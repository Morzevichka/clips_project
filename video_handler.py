...
class VideoProcessing:
    ...
    def process(self, video_id: str, values_per_second: int, width: int = 640, height: int = 360):
        ...
        frames, fps = self.get_len_fps(video_id)
        ...
        if len(video_frames) == cap.get(cv2.CAP_PROP_FRAME_COUNT) // (fps // values_per_second):
            return np.array(video_frames).mean(axis=2, keepdims=True)
        else:
